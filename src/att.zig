...
test {
  ...
  var att = try ATT.init(std.testing.allocator, 1);
  defer att.deinit();

  var handle = try att.registerAttribute(.{.short = 0xab}, 5);
  try std.testing.expect(handle == @as(Handle, 1));

  var value = [5]u8{'a', 'b', 'c', 'd', 'e'};
  try att.writeAttributeByHandle(handle, &value);

  var data = att.readAttributebyHandle(handle).?;
